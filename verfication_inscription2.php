                ...
                if(strlen($email) <= 100){ // On verifie que la longueur du mail <= 100
                    if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // Si l'email est de la bonne forme
                        if($password === $password_retype){ // si les deux mdp saisis sont bon
                            ...
                            header('Location:connexion2.php?reg_err=success');
                            die();
                        }else{ header('Location: inscription2.php?reg_err=password'); die();}
                    }else{ header('Location: inscription2.php?reg_err=email'); die();}
                }else{ header('Location: inscription2.php?reg_err=email_length'); die();}
